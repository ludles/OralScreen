...
BATCH_SIZE = 128
EPOCHS = 30
CSV_PATH = './fold_slide_results_data1.csv'
...
writer = csv.writer(f)
header = ['Fold', 'Slide', 'Class', 'Accuracy', 'Percentage of cancer prediction', 'STD']
...
for fold in tqdm(['1', '2', '3']):
    ...
    for i_model in ['1', '2', '3']:
        ...
        WEIGHT_PATH = DIR_LOG + "ResNet_aug" + i_model + ".hdf5"
    ...
    #        print("{}\t {}\t {:.5f}\t {:.5f}".format(i_slide, slide_class, acc_slide, perc_cancer))
            results_all.append([i_slide, perc_cancer])
        
    for i_slide in slides_test:
        results = np.asarray([line[-1] for line in results_all if i_slide in line])
        ...
        if i_slide in slides_test0:
            ...
            slide_class = 'Healthy'
            acc_slide = 1 - average
        writer.writerow([fold, i_slide, slide_class, acc_slide, average, std])
...
def read_csv_data(path):
    with open(path, 'r', newline='') as f:
        f_scv = csv.reader(f)
        header = next(f_scv)
        glasses = list(set([line[header.index('Slide')] for line in f_scv]))
    glasses.sort()
    
    aves = [[0] * len(glasses) for i in range(3)]
    stds = [[0] * len(glasses) for i in range(3)]

    with open(path, 'r', newline='') as f:
        f_scv = csv.reader(f)
        header = next(f_scv)
        for line in f_scv:
            i_fold = int(line[header.index('Fold')]) - 1
            i_slide = glasses.index(line[header.index('Slide')])
            perc_cancer = line[header.index('Percentage of cancer prediction')]
            aves[i_fold][i_slide] = float(perc_cancer)
            std = line[header.index('STD')]
            stds[i_fold][i_slide] = float(std)
            
    return aves, stds, glasses

aves, stds, glasses = read_csv_data(CSV_PATH)

with open(CSV_PATH, 'a+', newline='') as f:
    writer = csv.writer(f)
    writer.writerow([''] + glasses + ['STDs'])
    for i_fold in range(len(aves)):
        writer.writerow(['fold{}'.format(i_fold + 1)] + aves[i_fold] + stds[i_fold])